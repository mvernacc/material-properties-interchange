...
class StateDependentProperty(Property):
    ...
    def __init__(self, yaml_dict):
        ...
        if self._representation == 'table':
            if len(self.state_vars) == 1:
                ...
                interp_points_0 = np.array(tbl_dict.keys())
                ...
                for i in len(interp_points_0):
                    # The value of the 0th state variable for this slice of the inperpolation table.
                    point_0 = interp_points_0[i]
                    self._interp_points.append(
                        self._interp_points,
                        np.concatenate(
                            (
                                np.full_like(interp_points_1[i], point_0),
                                interp_points_1[i]
                            ),
                            axis=1),
                        axis=0)
                    self._interp_values.append(tbl_dict[point_0]['values'])
            else:
    ...
    def query_value(self, state, method='linear', fill_value=np.nan, rescale=False):
        ...
        if len(self.state_vars) == 1:
            ...
            assert len(state[self.state_vars[0]]) == state[self.state_vars[1]]
        ...
        # return a scalar if we were given a scalar
